<template>
  <q-page-sticky position="bottom-right" class="c-locale-switch" :offset="[18, 18]">
    <q-fab v-model="fab" :label="$t('locale.title')" external-label vertical-actions-align="right" :icon="icon" direction="up" >
      <q-fab-action label-position="left" icon="img:flags/pt-BR.svg" @click="set('pt-BR')" :label="$t('locale.ptbr')" external-label />
      <q-fab-action label-position="left" icon="img:flags/en-US.svg" @click="set('en-US')" :label="$t('locale.enus')" external-label />
    </q-fab>
  </q-page-sticky>
</template>
